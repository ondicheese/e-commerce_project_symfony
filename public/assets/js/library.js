    ...
    if (tbody) {
        ...
        if (compare) {
            ...
            compare.forEach((product) => {
                ...
                <span class="price">${formatPrice(product.salePrice / 100)}</span></td>
                `
                addToCart.innerHTML += `
                <td class="row_btn">
...
export const addCartEventListenerToLink = () => {
    ...
    let add_to_cart_links = document.querySelectorAll('a.add-to-cart, a.item_remove,  a.btn-addtocart, li.add-to-cart a')
...
export const displayCart = (cart = null) => {
    ...
    let cart_sub_total_amount_ttc = document.querySelector('.cart_sub_total_amount_ttc')
    let cart_sub_total_amount_ht = document.querySelector('.cart_sub_total_amount_ht')
    ...
    let cart_tax = document.querySelector('.cart_tax')

    if (tbody) {
        tbody.innerHTML = ""
        cart.items.forEach((item) => {
            let { product, quantity, sub_total_ht, tax, sub_total } = item
            let content = `
             <tr>
                                     ...
                                     <td data-title="Price" class="product-price">
                                         ${formatPrice(product.salePrice / 100)}
                                     </td>
                                     ...
                                     <td data-title="Tax" class="product-subtotal">
                                         ${formatPrice(tax)} </td>
                                     <td data-title="Total_excl_taxes" class="product-subtotal">
                                         ${formatPrice(sub_total_ht)} </td>
                                     <td data-title="Total_incl_taxes" class="product-subtotal">
                                         ${formatPrice(sub_total)} </td>
        ...
        cart_sub_total_amount_ht.innerHTML = formatPrice(cart.sub_total_ht)
        cart_sub_total_amount_ttc.innerHTML = formatPrice(cart.sub_total)
        cart_tax.innerHTML = formatPrice(cart.tax)
        cart_total_amount.innerHTML = formatPrice(cart.sub_total + cart.carrier.price)
        cart_shipping_total_amount.innerHTML = formatPrice(cart.carrier.price)
    }
    console.log(cart)
    addCartEventListenerToLink()
}
export const displayWishlist = (wishlist = null) => {
    ...
    if (tbody) {
        ...
        wishlist.forEach((product) => {
            ...
            <td data-title="Product" class="product-name"><a href="/product/${product.slug}">
                    ${ product.name }
                </a>
            </td>
            <td data-title="Price" class="product-price">
...
export const updateHeaderCart = async (cart = null) => {
    ...
    let cart_tax = document.querySelector(".header_cart_tax")
    ...
    console.log(cart.sub_total_ht, cart.tax, cart.sub_total)
    // cart data found
    cart_count ? cart_count.innerHTML = cart?.cart_count : null
    cart_price_value_ht ? cart_price_value_ht.innerHTML = formatPrice(cart.sub_total_ht) : 0
    cart_tax ? cart_tax.innerHTML = formatPrice(cart.tax) : null
    cart_price_value ? cart_price_value.innerHTML = formatPrice(cart.sub_total) : null
            ...
            cart_list.innerHTML += `
                ...
                <span class="cart_quantity"> ${quantity} x
                    <span class="cart_amount">
                        <span class="price_symbole">${formatPrice(product.salePrice / 100)} (${formatPrice(product.salePrice / 100 * quantity)})</span>
                    </span>
                </span>
            </li>
                `
...
export const addActiveInClass = (pageNb) => {
    const pageLinks = document.querySelectorAll('.page-link');

    pageLinks.forEach(link => {
        if (link.innerHTML === pageNb) {
            link.classList.add('active')
            link.style.backgroundColor = '#FF324D'
            link.style.borderColor = '#FF324D'
